...
def load_img(filepath):
    img = Image.open(filepath).convert('L')    # 将图像转换成灰度图
    # y, _, _ = img.split()
    return img
...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = load_img(self.image_filenames[index])   # 将图像转换为灰度图
        target, _ = get_patch(target, self.patch_size, self.upscale_factor)  # 目标图像
